...
- **Diner (consumer)** — browses, claims, pays. Mobile-first iOS-style web app at `/app`.
- **Merchant** — restaurant operator creating and managing offers, monitoring redemptions, managing settlement. Desktop-friendly web app at `/dashboard`.
- **Admin (ops)** — internal MealMate team approving merchants, reviewing flagged claims, monitoring fraud signals, reconciling payments. Web app at `/admin`.

---

## Architecture decisions (immutable — don't re-litigate)

- **Single Next.js 16 monorepo.** Three role-gated sections under the app router:
  - `/app` → diner
  - `/dashboard` → merchant
  - `/admin` → ops
...
- Social (Apple, Google) for diners — TBD, deferred until Phase 2+
...
- Auth UI for sign-up and sign-in (single form, role picker for diner / merchant)
- Three roles enforced: redirect after login based on `users.role` (`diner` → `/app`, `merchant` → `/dashboard`, `admin` → `/admin`)
...
- **Consumer:** `mealmate-jet.vercel.app` (repo: `mealmate`) — 32 screens, Dallas-localized, Bishop Arts focus. Visual reference for `/app` (diner section) in this monorepo.
